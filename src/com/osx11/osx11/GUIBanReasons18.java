...
public class GUIBanReasons18 {

    private Main plugin;

    public GUIBanReasons18(Main plugin) {
        this.plugin = plugin;
    }
    ...
    public void openGUIBanReasons(Player player) {
        ...
        try {
            Inventory inventory = Bukkit.createInventory(null, 18, plugin.getConfig().getString("messages.GUIBanReasonsName").replace("&", "§"));

            int itemNumber = plugin.getConfig().getConfigurationSection("banreasons").getKeys(false).toArray().length;
            int totalNumber = 18; // всего
            int itemNumber2 = -1; // сколько надо отнять
            int itemNumber3; // сколько раз повторить цикл
            String[] displayItem = new String[18];
            String[] reason = new String[18];
            ...
            for (int i = 0; i < itemNumber3; i++) { // Если прични для бана в конфиге меньше 18
